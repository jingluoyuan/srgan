...
class Application(tk.Frame):
    ...
    def get_face_img(self):
        ...
        img1 = Image.open(self.img_path)
        img2 = Image.open('results/test_srgan.jpg')
        width1, height1 = img1.size
        img2=img2.resize((width1,height1))
        cropped_img1 = img1.crop(self.face_coords)
        cropped_img1 = cropped_img1.convert("RGB")
        cropped_img2 = img2.crop(self.face_coords)
        cropped_img2 = cropped_img2.convert("RGB")
        face_path1 = "face1.jpg"
        face_path2 = "face2.jpg"
        cropped_img1.save(face_path1)
        cropped_img2.save(face_path2)
        self.face_path = face_path1
        self.spare_img = face_path2
        return face_path1,face_path2
    ...
    def compare(self):
        ...
        face_path,spare_path = self.get_face_img()
        if not face_path:
            return

        # 将圈出的人脸区域传入SRGAN模型进行超分辨率重建
        self.reconstructed_img = 'results/test_srgan.jpg'
        ...
        # 打开两张指定路径的图片
        image1 = Image.open(face_path)
        image2 = Image.open(spare_path)
    ...
    def reconstruct(self):
        ...
        if self.hr_img is None:
            messagebox.showerror("错误", "超分辨率重建失败")
            return
        self.hr_img = Image.open(self.spare_img)
        self.hr_img = self.hr_img.convert("RGB")
        self.hr_img = self.hr_img.resize((300,300))
